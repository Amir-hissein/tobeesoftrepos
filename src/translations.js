export const translations = {
    fr: {
        navbar: {
            home: "Accueil",
            about: "À Propos",
            services: "Services",
            process: "Processus",
            expertise: "Expertise",
            faq: "FAQ",
            contact: "Contact"
        },
        hero: {
            title: "Transformez votre vision digitale en réalité",
            subtitle: "Votre partenaire expert en développement logiciel et numérisation des infrastructures technologiques",
            description: "Nous concevons des solutions logicielles innovantes et sur mesure qui propulsent votre entreprise vers l'excellence numérique. De l'idée initiale au déploiement final, nous transformons vos défis technologiques en opportunités de croissance.",
            startProject: "Démarrer votre projet",
            discoverServices: "Découvrir nos services",
            stats: {
                projects: "Projets réalisés",
                satisfaction: "Satisfaction",
                support: "Support 24/7"
            }
        },
        about: {
            label: "Qui sommes-nous",
            title: "À Propos de TOBEESOFT",
            history: {
                title: "Notre Histoire",
                content: "Tobeesoft est née d'une passion commune pour l'innovation technologique et d'une volonté de rendre la transformation digitale accessible à toutes les entreprises. Depuis notre création, nous avons accompagné des dizaines d'organisations dans leur évolution numérique."
            },
            mission: {
                title: "Notre Mission",
                content: "Nous croyons fermement que chaque entreprise mérite d'avoir accès à des solutions technologiques de pointe. Notre mission est de démocratiser l'excellence logicielle en proposant des services de développement qui allient qualité, innovation et accessibilité."
            },
            vision: {
                title: "Notre Vision",
                content: "Devenir le partenaire de référence des entreprises qui souhaitent accélérer leur transformation digitale et construire un avenir technologique durable, performant et évolutif."
            },
            valuesTitle: "Nos Valeurs",
            values: {
                excellence: {
                    title: "Excellence Technique",
                    content: "Nous nous engageons à livrer des solutions de la plus haute qualité, en utilisant les technologies les plus récentes."
                },
                listening: {
                    title: "Écoute et Transparence",
                    content: "Votre réussite est notre priorité. Nous écoutons attentivement vos besoins et maintenons une communication claire."
                },
                innovation: {
                    title: "Innovation Continue",
                    content: "Le monde technologique évolue rapidement. Nous restons constamment à la pointe de l'innovation."
                },
                partnership: {
                    title: "Partenariat Durable",
                    content: "Nous ne sommes pas qu'un simple prestataire, nous sommes votre partenaire à long terme."
                }
            }
        },
        services: {
            label: "Ce que nous faisons",
            title: "Nos Services",
            service1: {
                title: "Développement Logiciel sur Mesure",
                webApps: {
                    title: "Applications Web",
                    description: "Création d'applications web modernes, réactives et performantes."
                },
                mobileApps: {
                    title: "Applications Mobiles",
                    description: "Développement d'applications mobiles natives et cross-platform."
                },
                desktopApps: {
                    title: "Applications Desktop",
                    description: "Conception de logiciels desktop puissants et intuitifs."
                },
                businessSoftware: {
                    title: "Logiciels Métier",
                    description: "Solutions spécialement conçues pour votre secteur (ERP, CRM)."
                }
            },
            service2: {
                title: "Numérisation des Infrastructures",
                audit: {
                    title: "Audit et Diagnostic",
                    description: "Analyse approfondie de votre infrastructure."
                },
                migration: {
                    title: "Migration vers le Cloud",
                    description: "Transition sécurisée vers des solutions cloud."
                },
                automation: {
                    title: "Automatisation",
                    description: "Mise en place de workflows automatisés."
                },
                modernization: {
                    title: "Modernisation Legacy",
                    description: "Transformation de vos anciens systèmes."
                }
            },
            service3: {
                title: "Intégration de Systèmes",
                api: {
                    title: "API et Microservices",
                    description: "Développement d'APIs robustes."
                },
                erp: {
                    title: "Intégration ERP/CRM",
                    description: "Connexion de vos outils de gestion."
                },
                database: {
                    title: "Bases de Données",
                    description: "Synchronisation et migration de données."
                }
            },
            service4: {
                title: "Conseil et Stratégie",
                techAudit: {
                    title: "Audit Technologique",
                    description: "Évaluation complète de votre maturité digitale."
                },
                roadmap: {
                    title: "Feuille de Route",
                    description: "Stratégie de transformation numérique sur mesure."
                },
                architecture: {
                    title: "Architecture",
                    description: "Conception de l'architecture technique optimale."
                }
            },
            service5: {
                title: "Maintenance et Support",
                corrective: {
                    title: "Maintenance Corrective",
                    description: "Correction rapide des bugs."
                },
                evolutive: {
                    title: "Maintenance Évolutive",
                    description: "Ajout de nouvelles fonctionnalités."
                },
                support247: {
                    title: "Support 24/7",
                    description: "Assistance technique réactive."
                }
            }
        },
        process: {
            label: "Comment nous travaillons",
            title: "Notre Processus de Travail",
            steps: [
                {
                    title: "Découverte et Analyse",
                    duration: "1-2 semaines",
                    items: [
                        "Rencontre initiale pour comprendre votre contexte",
                        "Analyse approfondie des besoins fonctionnels",
                        "Étude de faisabilité technique et financière",
                        "Proposition détaillée avec périmètre et budget"
                    ]
                },
                {
                    title: "Conception et Design",
                    duration: "2-3 semaines",
                    items: [
                        "Architecture technique et choix des technologies",
                        "Wireframes et maquettes UI/UX",
                        "Spécifications techniques détaillées",
                        "Validation client avant développement"
                    ]
                },
                {
                    title: "Développement",
                    duration: "Variable selon projet",
                    items: [
                        "Sprints agiles avec livraisons régulières",
                        "Revues hebdomadaires ou bi-hebdomadaires",
                        "Tests continus (unitaires, intégration)",
                        "Code reviews systématiques"
                    ]
                },
                {
                    title: "Tests et Validation",
                    duration: "1-2 semaines",
                    items: [
                        "Tests fonctionnels complets",
                        "Tests de performance et charge",
                        "Audit de sécurité",
                        "Recette utilisateur avec accompagnement"
                    ]
                },
                {
                    title: "Déploiement",
                    duration: "1 semaine",
                    items: [
                        "Préparation de l'environnement de production",
                        "Migration des données si nécessaire",
                        "Mise en production sécurisée",
                        "Formation des utilisateurs"
                    ]
                },
                {
                    title: "Support et Évolution",
                    duration: "Continue",
                    items: [
                        "Période de garantie avec support inclus",
                        "Monitoring et surveillance en temps réel",
                        "Maintenance évolutive",
                        "Mises à jour de sécurité régulières"
                    ]
                }
            ]
        },
        expertise: {
            label: "Nos compétences",
            title: "Nos Domaines d'Expertise",
            industries: {
                title: "Secteurs d'Activité",
                commerce: "Commerce",
                health: "Santé",
                education: "Éducation",
                finance: "Finance",
                industry: "Industrie",
                logistics: "Logistique",
                realEstate: "Immobilier",
                hr: "RH"
            },
            technologies: {
                title: "Technologies et Compétences",
                frontend: "Frontend & UI/UX",
                backend: "Backend & API",
                mobile: "Mobile",
                database: "Base de Données",
                cloud: "Cloud & DevOps",
                ai: "IA & Data"
            }
        },
        faq: {
            label: "Vos questions",
            title: "Questions Fréquentes",
            questions: [
                {
                    question: "Combien coûte le développement d'une application ?",
                    answer: "Le coût dépend de nombreux facteurs : complexité fonctionnelle, technologies utilisées, design, intégrations tierces, etc. Nous proposons toujours un devis personnalisé après avoir analysé vos besoins."
                },
                {
                    question: "Combien de temps prend le développement d'un projet ?",
                    answer: "La durée varie selon la complexité. Un projet simple peut prendre 1-2 mois, tandis qu'un projet complexe peut nécessiter 6-12 mois ou plus."
                },
                {
                    question: "Proposez-vous des contrats de maintenance ?",
                    answer: "Oui, nous proposons différentes formules de maintenance et support : corrective, évolutive, préventive. Les contrats sont adaptables selon vos besoins."
                },
                {
                    question: "Puis-je voir des exemples de vos réalisations ?",
                    answer: "Absolument ! Nous pouvons vous présenter des études de cas et des démos de projets similaires au vôtre."
                },
                {
                    question: "Travaillez-vous en régie ou au forfait ?",
                    answer: "Nous proposons les deux modèles selon vos préférences. Le forfait est idéal pour des projets avec un périmètre bien défini."
                },
                {
                    question: "Offrez-vous des garanties sur vos développements ?",
                    answer: "Oui, tous nos projets incluent une période de garantie pendant laquelle nous corrigeons gratuitement les bugs."
                },
                {
                    question: "Puis-je récupérer le code source ?",
                    answer: "Oui, vous êtes propriétaire du code source développé pour votre projet. Nous vous remettons l'intégralité du code."
                },
                {
                    question: "Comment gérez-vous la confidentialité ?",
                    answer: "Nous signons systématiquement des accords de confidentialité (NDA) avant tout échange d'informations sensibles."
                }
            ]
        },
        contact: {
            label: "Parlons de votre projet",
            title: "Contactez-nous",
            info: {
                title: "Informations de Contact",
                email: "Email",
                phone: "Téléphone",
                address: "Adresse",
                hours: "Horaires d'Ouverture",
                hoursValue: "Lundi - Vendredi : 9h00 - 18h00"
            },
            form: {
                title: "Envoyez-nous un message",
                name: "Nom complet",
                email: "Email",
                phone: "Téléphone",
                company: "Entreprise",
                projectType: "Type de projet",
                message: "Message",
                placeholders: {
                    name: "Votre nom",
                    email: "votre@email.com",
                    phone: "+33 6...",
                    company: "Nom de votre société",
                    message: "Décrivez votre projet..."
                },
                selectPlaceholder: "Sélectionnez une option",
                projectTypes: {
                    web: "Développement Web",
                    mobile: "Application Mobile",
                    software: "Logiciel Sur Mesure",
                    cloud: "Cloud & DevOps",
                    audit: "Audit & Conseil",
                    other: "Autre"
                },
                submit: "Envoyer le message",
                sending: "Envoi en cours...",
                success: " Merci ! Votre message a été envoyé avec succès.",
                error: "❌ Erreur:"
            }
        },
        footer: {
            description: "Partenaire de confiance pour votre transformation digitale. Nous créons des solutions logicielles innovantes pour propulser votre entreprise vers l'avenir.",
            services: {
                title: "Services",
                web: "Développement Web",
                mobile: "Applications Mobiles",
                cloud: "Cloud & DevOps",
                consulting: "Conseil & Audit"
            },
            links: {
                title: "Liens Utiles",
                about: "À Propos",
                process: "Processus",
                expertise: "Expertise",
                contact: "Contact"
            },
            contact: {
                title: "Contact"
            },
            copyright: "Tous droits réservés."
        }
    },
    en: {
        navbar: {
            home: "Home",
            about: "About",
            services: "Services",
            process: "Process",
            expertise: "Expertise",
            faq: "FAQ",
            contact: "Contact"
        },
        hero: {
            title: "Transform your digital vision into reality",
            subtitle: "Your expert partner in software development and technological infrastructure digitalization",
            description: "We design innovative and custom software solutions that propel your business towards digital excellence. From initial idea to final deployment, we transform your technological challenges into growth opportunities.",
            startProject: "Start your project",
            discoverServices: "Discover our services",
            stats: {
                projects: "Projects completed",
                satisfaction: "Satisfaction",
                support: "24/7 Support"
            }
        },
        about: {
            label: "Who we are",
            title: "About TOBEESOFT",
            history: {
                title: "Our Story",
                content: "Tobeesoft was born from a shared passion for technological innovation and a desire to make digital transformation accessible to all businesses. Since our inception, we have supported dozens of organizations in their digital evolution."
            },
            mission: {
                title: "Our Mission",
                content: "We firmly believe that every company deserves access to cutting-edge technological solutions. Our mission is to democratize software excellence by offering development services that combine quality, innovation, and accessibility."
            },
            vision: {
                title: "Our Vision",
                content: "To become the reference partner for companies wishing to accelerate their digital transformation and build a sustainable, high-performance, and scalable technological future."
            },
            valuesTitle: "Our Values",
            values: {
                excellence: {
                    title: "Technical Excellence",
                    content: "We are committed to delivering the highest quality solutions, using the latest technologies."
                },
                listening: {
                    title: "Listening & Transparency",
                    content: "Your success is our priority. We listen carefully to your needs and maintain clear communication."
                },
                innovation: {
                    title: "Continuous Innovation",
                    content: "The technological world evolves rapidly. We constantly stay at the forefront of innovation."
                },
                partnership: {
                    title: "Sustainable Partnership",
                    content: "We are not just a service provider, we are your long-term partner."
                }
            }
        },
        services: {
            label: "What we do",
            title: "Our Services",
            service1: {
                title: "Custom Software Development",
                webApps: {
                    title: "Web Applications",
                    description: "Creation of modern, responsive, and high-performance web applications."
                },
                mobileApps: {
                    title: "Mobile Applications",
                    description: "Development of native and cross-platform mobile applications."
                },
                desktopApps: {
                    title: "Desktop Applications",
                    description: "Design of powerful and intuitive desktop software."
                },
                businessSoftware: {
                    title: "Business Software",
                    description: "Solutions specially designed for your sector (ERP, CRM)."
                }
            },
            service2: {
                title: "Infrastructure Digitalization",
                audit: {
                    title: "Audit & Diagnostics",
                    description: "In-depth analysis of your infrastructure."
                },
                migration: {
                    title: "Cloud Migration",
                    description: "Secure transition to cloud solutions."
                },
                automation: {
                    title: "Automation",
                    description: "Implementation of automated workflows."
                },
                modernization: {
                    title: "Legacy Modernization",
                    description: "Transformation of your legacy systems."
                }
            },
            service3: {
                title: "Systems Integration",
                api: {
                    title: "API & Microservices",
                    description: "Development of robust APIs."
                },
                erp: {
                    title: "ERP/CRM Integration",
                    description: "Connection of your management tools."
                },
                database: {
                    title: "Databases",
                    description: "Data synchronization and migration."
                }
            },
            service4: {
                title: "Consulting & Strategy",
                techAudit: {
                    title: "Technology Audit",
                    description: "Complete assessment of your digital maturity."
                },
                roadmap: {
                    title: "Roadmap",
                    description: "Custom digital transformation strategy."
                },
                architecture: {
                    title: "Architecture",
                    description: "Design of optimal technical architecture."
                }
            },
            service5: {
                title: "Maintenance & Support",
                corrective: {
                    title: "Corrective Maintenance",
                    description: "Quick bug fixes."
                },
                evolutive: {
                    title: "Evolutive Maintenance",
                    description: "Addition of new features."
                },
                support247: {
                    title: "24/7 Support",
                    description: "Responsive technical assistance."
                }
            }
        },
        process: {
            label: "How we work",
            title: "Our Work Process",
            steps: [
                {
                    title: "Discovery & Analysis",
                    duration: "1-2 weeks",
                    items: [
                        "Initial meeting to understand your context",
                        "In-depth analysis of functional requirements",
                        "Technical and financial feasibility study",
                        "Detailed proposal with scope and budget"
                    ]
                },
                {
                    title: "Design & Planning",
                    duration: "2-3 weeks",
                    items: [
                        "Technical architecture and technology choices",
                        "Wireframes and UI/UX mockups",
                        "Detailed technical specifications",
                        "Client validation before development"
                    ]
                },
                {
                    title: "Development",
                    duration: "Variable by project",
                    items: [
                        "Agile sprints with regular deliveries",
                        "Weekly or bi-weekly reviews",
                        "Continuous testing (unit, integration)",
                        "Systematic code reviews"
                    ]
                },
                {
                    title: "Testing & Validation",
                    duration: "1-2 weeks",
                    items: [
                        "Complete functional testing",
                        "Performance and load testing",
                        "Security audit",
                        "User acceptance with support"
                    ]
                },
                {
                    title: "Deployment",
                    duration: "1 week",
                    items: [
                        "Production environment preparation",
                        "Data migration if necessary",
                        "Secure production deployment",
                        "User training"
                    ]
                },
                {
                    title: "Support & Evolution",
                    duration: "Ongoing",
                    items: [
                        "Warranty period with included support",
                        "Real-time monitoring and surveillance",
                        "Evolutive maintenance",
                        "Regular security updates"
                    ]
                }
            ]
        },
        expertise: {
            label: "Our skills",
            title: "Our Areas of Expertise",
            industries: {
                title: "Industry Sectors",
                commerce: "E-commerce",
                health: "Healthcare",
                education: "Education",
                finance: "Finance",
                industry: "Industry",
                logistics: "Logistics",
                realEstate: "Real Estate",
                hr: "HR"
            },
            technologies: {
                title: "Technologies & Skills",
                frontend: "Frontend & UI/UX",
                backend: "Backend & API",
                mobile: "Mobile",
                database: "Database",
                cloud: "Cloud & DevOps",
                ai: "AI & Data"
            }
        },
        faq: {
            label: "Your questions",
            title: "Frequently Asked Questions",
            questions: [
                {
                    question: "How much does application development cost?",
                    answer: "The cost depends on many factors: functional complexity, technologies used, design, third-party integrations, etc. We always provide a personalized quote after analyzing your needs."
                },
                {
                    question: "How long does project development take?",
                    answer: "Duration varies based on complexity. A simple project may take 1-2 months, while a complex project may require 6-12 months or more."
                },
                {
                    question: "Do you offer maintenance contracts?",
                    answer: "Yes, we offer different maintenance and support packages: corrective, evolutive, preventive. Contracts are adaptable to your needs."
                },
                {
                    question: "Can I see examples of your work?",
                    answer: "Absolutely! We can present case studies and demos of projects similar to yours."
                },
                {
                    question: "Do you work on a time-and-materials or fixed-price basis?",
                    answer: "We offer both models according to your preferences. Fixed-price is ideal for projects with a well-defined scope."
                },
                {
                    question: "Do you offer warranties on your developments?",
                    answer: "Yes, all our projects include a warranty period during which we fix bugs free of charge."
                },
                {
                    question: "Can I get the source code?",
                    answer: "Yes, you own the source code developed for your project. We provide you with the complete code."
                },
                {
                    question: "How do you handle confidentiality?",
                    answer: "We systematically sign non-disclosure agreements (NDA) before any exchange of sensitive information."
                }
            ]
        },
        contact: {
            label: "Let's talk about your project",
            title: "Contact Us",
            info: {
                title: "Contact Information",
                email: "Email",
                phone: "Phone",
                address: "Address",
                hours: "Opening Hours",
                hoursValue: "Monday - Friday: 9:00 AM - 6:00 PM"
            },
            form: {
                title: "Send us a message",
                name: "Full name",
                email: "Email",
                phone: "Phone",
                company: "Company",
                projectType: "Project type",
                message: "Message",
                placeholders: {
                    name: "Your name",
                    email: "your@email.com",
                    phone: "+1 ...",
                    company: "Your company name",
                    message: "Describe your project..."
                },
                selectPlaceholder: "Select an option",
                projectTypes: {
                    web: "Web Development",
                    mobile: "Mobile Application",
                    software: "Custom Software",
                    cloud: "Cloud & DevOps",
                    audit: "Audit & Consulting",
                    other: "Other"
                },
                submit: "Send message",
                sending: "Sending...",
                success: " Thank you! Your message has been sent successfully.",
                error: "❌ Error:"
            }
        },
        footer: {
            description: "Trusted partner for your digital transformation. We create innovative software solutions to propel your business into the future.",
            services: {
                title: "Services",
                web: "Web Development",
                mobile: "Mobile Applications",
                cloud: "Cloud & DevOps",
                consulting: "Consulting & Audit"
            },
            links: {
                title: "Useful Links",
                about: "About",
                process: "Process",
                expertise: "Expertise",
                contact: "Contact"
            },
            contact: {
                title: "Contact"
            },
            copyright: "All rights reserved."
        }
    }
};
